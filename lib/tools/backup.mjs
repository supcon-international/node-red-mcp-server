...
export default function registerBackupTools(server, config) {
  ...
  server.tool(
    "restore-flows",
    "Restore Node-RED flows from a specific backup",
    {
      name: z
        .string()
        .optional()
        .describe(
          "Backup name to restore from (optional, uses latest if not provided)"
        ),
      confirm: z
        .boolean()
        .optional()
        .describe("Confirmation to proceed with restore operation"),
    },
    async ({ name, confirm }) => {
      try {
        if (!confirm) {
          const metadata = await getBackupMetadata(name, config);
          return {
            content: [
              {
                type: "text",
                text: `Ready to restore backup:\n\nName: ${metadata.name}\nCreated: ${metadata.timestamp}\nReason: ${metadata.reason}\nFlows: ${metadata.flowsCount} tabs, ${metadata.nodesCount} nodes\n\n⚠️  This will overwrite your current flows configuration!\nPlease call restore-flows again with confirm=true to proceed.`,
              },
            ],
          };
        }

        await restoreBackup(name, config);

        return {
          content: [
            {
              type: "text",
              text: `Flows restored successfully!\n\n✅ Your Node-RED flows have been updated and deployed.`,
            },
          ],
        };
      } catch (error) {
 ...
 */
async function restoreBackup(backupName, config) {
  await ensureBackupDirectory(config);
  const { backupDir, metadataPath } = getPaths(config);
  ...
  if (backupName) {
    ...
    const latest = metadata.backups.sort(
      (a, b) => new Date(b.timestamp) - new Date(a.timestamp)
    )[0];
    const backupFile = path.join(backupDir, latest.filename);
    backupData = JSON.parse(await fs.readFile(backupFile, "utf8"));
  }
  ...
  // Restore flows via Node-RED API (immediate deployment)
  await callNodeRed("post", "/flows", backupData.flows, config);
}

/**
 * Get backup metadata for confirmation
 */
async function getBackupMetadata(backupName, config) {
  await ensureBackupDirectory(config);
  const { backupDir, metadataPath } = getPaths(config);
